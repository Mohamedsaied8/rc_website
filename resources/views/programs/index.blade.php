    ...
    <section class="relative z-10 max-w-6xl mx-auto px-6 py-16">
        <div class="flex flex-col gap-8">
            @foreach($programs as $program)
                <div class="bg-white shadow-sm border border-slate-200 rounded-2xl p-8 hover:border-cyan-400/20 transition-all duration-500">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                        <!-- Left side content -->
                        <div>
                            <h2 class="text-2xl font-bold text-slate-900 mb-3">{{ $program->title }}</h2>
                            ...
                            <div class="flex flex-wrap gap-1.5 mb-5">
                                ...
                                @foreach(array_slice($topics, 0, 4) as $topic)
                                    <span class="text-xs text-slate-600 bg-slate-50 border border-slate-200 px-2 py-1 rounded-md">{{ $topic }}</span>
                                @endforeach
                            </div>
                            
                            <div class="flex items-center gap-6 mt-5">
                                <div class="flex items-center gap-2 text-sm text-slate-600">
                                    <span class="text-cyan-600">💰</span> EGP {{ number_format($program->price) }}
                                </div>
                                <div class="flex items-center gap-2 text-sm text-slate-600">
                                    <span class="text-cyan-600">⏱️</span> {{ $program->duration }}
                                </div>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-3 mt-6">
                                ...
                                <a href="{{ route('programs.show', $program->slug) }}" class="px-6 py-2.5 text-sm font-semibold text-slate-900 border border-white/15 bg-white/[0.04] rounded-xl hover:border-white/25 transition-all duration-300">
                                    Program Details
                                </a>
                            </div>
                        </div>
